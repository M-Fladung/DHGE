...
class Point
{
  public:
    ...
    Point(const Color &color, int x, int y, int sx = 0, int sy = 0)
    {
...
int main(void)
{
  ...
  { 
    ...
    Point p(Color(255, 255, 255), SDL_X_SIZE / 2, SDL_Y_SIZE / 2, 2, 1);

    for (;;) {
      sdlMilliSleep(20);   // "Bremse"
      p.fly(true);    // Aufruf der Methode fly der Klasse Point
