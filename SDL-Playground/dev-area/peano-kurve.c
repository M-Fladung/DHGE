// Diagonale Peano-Kurve (Angabe: Hauptprogramm zum Erweitern)
//
...
void zeichne(int tiefe) {
    if (tiefe == 0) {
        // Base case: Draw a single line segment in the current direction
        int oldX = xPos, oldY = yPos;

        // Update position based on the current direction
        switch (richtung % 4) {
            case 0: xPos += schritt; yPos += schritt; break; // right-down (+x+y)
            case 1: xPos += schritt; yPos -= schritt; break; // right-up (+x-y)
            case 2: xPos -= schritt; yPos -= schritt; break; // left-up (-x-y)
            case 3: xPos -= schritt; yPos += schritt; break; // left-down (-x+y)
        }

    int farbe = richtung % (sizeof(R) / sizeof(R[0]));
    sdlDrawLine(oldX, oldY, xPos, yPos, R[farbe], G[farbe], B[farbe]);
        sdlUpdate();
